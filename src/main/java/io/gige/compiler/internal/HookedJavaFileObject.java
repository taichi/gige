...
/**
 * A delegating JavaFileObject that hooks the close() methods of the Writer or
...
public class HookedJavaFileObject extends
	...
	/**
	 * The Filer implementation that we need to notify when a new file is
	 * created.
	 */
	protected final FilerImpl _filer;

	/**
	 * The name of the file that is created; this is passed to the
	 * CompilationUnit constructor, and ultimately to the java.io.File
	 * constructor, so it is a normal pathname, just like what would be on the
	 * compiler command line.
	 */
	protected final String _fileName;

	/**
	 * A compilation unit is created when the writer or stream is closed. Only
	 * do this once.
	 */
	...
	public HookedJavaFileObject(JavaFileObject fileObject, String fileName,
			String typeName, FilerImpl filer) {
		super(fileObject);
		_filer = filer;
		_fileName = fileName;
		_typeName = typeName;
	...
	protected void closed() {
		if (!_closed) {
			...
			switch (this.getKind()) {
			case SOURCE:
				CompilationUnit unit = new CompilationUnit(null, _fileName,
						null /* encoding */);
